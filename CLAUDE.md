...
#### GPTZero Data Accuracy Fix (v1.3.0 - February 2026)

- **Root cause:** The plagiarism plugin's `api.php` `transform_response()` method stripped `sentences[]` and `class_probabilities` from GPTZero API responses, leaving only document-level `predicted_class` and a single `class_probability` value. This caused two problems:
  1. `aireport.php` had to rescan documents (double API call, double quota usage)
  2. The stats grid showed misleading sentence-level counts using arbitrary thresholds (generated_prob ≥ 0.8) that didn't match GPTZero's document-level classification
- **Example of the bug:** A document GPTZero classified as "Human Generated" (8% AI / 4% Mixed / 88% Human) was shown as "34% AI" in the plugin because 38 of 111 sentences had individual `generated_prob ≥ 0.8`
- **aicheck.php fix:** Now calls GPTZero API directly (bypassing `transform_response()`) to get the full response including `sentences[]`, `class_probabilities`, and `completely_generated_prob`. The complete response is stored in `scanurl` field so `aireport.php` uses cached data without rescanning.
- **aireport.php fix (both versions):** Stats grid now shows GPTZero's authoritative document-level `class_probabilities` (AI %, Mixed %, Human %) instead of computed sentence-level counts. Sentence highlighting is retained as a visual aid with per-sentence `generated_prob` tooltips.
- **draftfeedback/aireport.php fix:** Added 3-pill probability display (AI/Mixed/Human) matching the learner version. Stats grid updated identically.
- **Files changed:** `local/learner/aicheck.php`, `local/learner/aireport.php`, `local/draftfeedback/aireport.php`
...
| #8      | Hardcoded Table Prefixes (mdl\_)         | MEDIUM              | Diagnosed       |
...
## Custom Login & Front Page (February 2026)

### Design System

Modern glassmorphism design with animated inspirational quotes:
...
- Text primary: `#1e293b`
- Fonts: Inter (body), Libre Baskerville (headings)
...
- **Animated Quotes:** 5 inspirational quotes cycle every 5 seconds with fade animation
- **Glassmorphism Cards:** `backdrop-filter: blur(10px)` with subtle shadows
- **Responsive:** Mobile-optimized (navbar hidden on mobile, content reordered)
- **Split Layout:** Left side has quotes/branding, right side has login form
...
- **Plugin removal cleanup:** When deleting a Moodle plugin folder manually, must also remove entries from `config_plugins` table or Moodle throws "error communicating with server"
- **CSS vs inline styles:** Moodle templates with inline `style` attributes cannot be overridden by external CSS (even with `!important`). Requires JavaScript or template modification.
- **Node version for Grunt:** Moodle 5.x requires Node >=22.11.0 <23. Higher versions (e.g., 25.x) fail with "Node version not satisfied"
