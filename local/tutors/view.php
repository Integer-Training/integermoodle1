...
if($fromform && !$action){
    ...
    //
    if($fromform->tutor){
        $sql .= "  AND ag.grader =".$fromform->tutor." ";
    }
    //
    if($fromform->startdate){ 
        $startdatearray = $fromform->startdate;
        //
        $starttimestamp = mktime(
            0, 0, 0,
            $startdatearray['month'],
            $startdatearray['day'],
            $startdatearray['year']
        );
        
        $sql .= "  AND ag.timemodified >= ".$starttimestamp."";
    } 
    //
    if($fromform->enddate){ 
        $enddatearray = $fromform->enddate;
        //
        $endtimestamp = mktime(
            23, 59, 59,
            $enddatearray['month'],
        ...
        $sql .= "  AND ag.timemodified <= ".$endtimestamp."";
    } 
    $sql .= "  group by userid,assignment";
    $sql .= "  order by ag.timemodified asc";
    //echo $sql;die;
    ...
    if($action){
        $sql = 'SELECT *  FROM {assign_grades} WHERE grader IS NOT NULL AND grader !=-1 AND grader ='.$USER->id.'
            group by userid,assignment';
    }else{
        $sql = 'SELECT *  FROM {assign_grades} WHERE grader IS NOT NULL AND grader !=-1 AND grader >2
            group by userid,assignment';
    }
...
//$records = $DB->get_records_sql($sql);
//if($conditions_sql){
    $records_sql = "SELECT ag.*  FROM {assign_grades} ag 
            JOIN {assign} a ON  a.id=ag.assignment
            JOIN {course} c ON c.id=a.course
            JOIN {course_modules} cm ON cm.course=c.id and cm.instance=a.id and cm.module=1
            WHERE ag.grader IS NOT NULL AND ag.grader !=-1 AND ag.grader >2
                 and cm.visible=1";
    $count_arr  = $DB->get_records_sql($records_sql);
    
    $main_count = count($count_arr);
//}
...
if($action){
    ...
    echo '<a href='.$r_url.' class="btn btn-primary"><- Back to course</a><br>';
}else{
    echo '<div class="card-header-lms">
                    <h4 class="m-b-0 text-white">
                        ...
                        <div class="pull-right">
                            <span class="label label-rounded label-info m-l-10 label-totals">'.$count_recs.'</span> of <span class="label label-rounded label-success m-l-10">'.$main_count.'</span>
                        </div>
                        <div class="pull-right mr-4" style="margin-top: -15px;">
                            <a href="#" class="btn btn-success waves-effect waves-light"><span class="btn-label"><i class="fa fa-file-excel-o"></i></span> Export CSV</a>
                        </div>
                    </h4>
                </div>';
}
