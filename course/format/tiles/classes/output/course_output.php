    ...
    protected function append_multi_section_page_data($data) {
        ...
        foreach ($secsall as $sectionnum => $section) {
            ...
            if ($sectionnum != 0 && $showsection && !$isdelegated) {
                ...
                $longtitlelength = 65;
                //coustomise by shiva
                $section_data = $DB->get_record('course_sections',['id'=>$section->id]);
                $section_data_arr = explode(',',$section_data->sequence);
                $first_sec_id = $section_data_arr[0];
                $cm = $DB->get_record('course_modules',['id'=>$first_sec_id,'course'=>$section_data->course]);
                $first_sec_name = $DB->get_field('hvp','name',['id'=>$cm->instance]);
                //echo $first_sec_name;die;
                //ends here

                $newtile = [
                    'tileid' => $section->section,
                    'secid' => $section->id,
                    'title' => $first_sec_name,
